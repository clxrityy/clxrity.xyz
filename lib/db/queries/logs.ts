...
export async function writeGuildLog(input: GuildLogInput) {
	...
	let messageCiphertext: Uint8Array<ArrayBuffer> | null | undefined;
	let detailsCiphertext: Uint8Array<ArrayBuffer> | null | undefined;
	...
	if (key && keyId) {
		if (input.message != null) {
			const raw = encryptAes256Gcm(
				typeof input.message === 'string' ? input.message : JSON.stringify(input.message),
				key
			);
			// Explicitly typed to ArrayBuffer to satisfy Prisma's Uint8Array<ArrayBuffer> constraint
			messageCiphertext = Uint8Array.from(raw) as Uint8Array<ArrayBuffer>;
		}
		if (input.details != null) {
			const text = typeof input.details === 'string' ? input.details : JSON.stringify(input.details);
			const raw = encryptAes256Gcm(text, key);
			detailsCiphertext = Uint8Array.from(raw) as Uint8Array<ArrayBuffer>;
		}
	}
